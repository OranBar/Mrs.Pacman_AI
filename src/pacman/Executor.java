...
import pacman.entries.pacman.TheRealPacmanController;
...
//		exec.runGameTimedSpeedOptimised(new MyMCTSPacMan(new AggressiveGhosts()),new AggressiveGhosts(),fixedTime,visual);
	...
	public void runGame(Controller<MOVE> pacManController,Controller<EnumMap<GHOST,MOVE>> ghostController,boolean visual,int delay)
	{
		...
		/*
		// Distance data gathering
		double maxDistance = Double.MIN_VALUE;
		
		for(Node firstNode : game.getCurrentMaze().graph){
			for(Node secondNode : game.getCurrentMaze().graph){
				double nodeDistance = game.getShortestPathDistance(firstNode.nodeIndex, secondNode.nodeIndex);
				if(maxDistance < nodeDistance){
